...
- **I3 (✅ منجز)**: **لوحة الأدمن** — `/admin`: إنشاء/تعديل (اسم/إيميل/هاتف/حالة)/تعطيل **عميل وموظف وموزّع**، تعيين الأدوار والفرع، إعادة كلمة المرور (مع إبطال الجلسات)، بحث وفلاتر، وسكربت `npm run make-admin` لدور Admin بكل الصلاحيات (29) · `/admin/branches`: CRUD للفروع مع إسناد المستودعات · `/roles`: **محرر صلاحيات** بصلاحية `roles.update` مع أقفال لأدوار النظام (409).
...
- **I5 (✅ منجز)**: لوحة الموزّع في `/pos` + `GET /api/pos/dashboard`: مبيعاته اليوم/الشهر (حدود اليوم محسوبة **بتوقيت الحساب** عبر `src/lib/time.ts`)، مخزون فرعه (المتاح/على الرف/منخفض/نافد بحد 10)، أخطر 5 نواقص، وأكثر 5 مبيعاً في الفرع خلال 30 يوماً، بالإضافة إلى شاشة البيع.
